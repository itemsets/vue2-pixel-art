...
<p align="center">
  <img src="https://github.com/itemsets/vue2-pixel-art/raw/master/image/mario.gif">
</p>
<p align="center">
...
- pixel.vue
  pixel.vue是一个组件（这里放着每个小小像素单位）；参数有`color`（RGB值）和`size`（像素大小）的两个数据，当它被点击并触发事件，就是通知其父组件并也将会触发一个事件。
- canvas.vue
